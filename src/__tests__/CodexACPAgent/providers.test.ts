...
describe("Configurable LLM providers (providers/*)", () => {
    ...
    it("restarts app-server and resumes all loaded sessions through the full provider lifecycle", async () => {
        ...
        const sideChats = (agent as unknown as {
            sideChats: {closeParent(sessionId: string): Promise<void>},
        }).sideChats;
        const closeSideChats = vi.spyOn(sideChats, "closeParent");
        ...
        expect(closeSideChats).toHaveBeenCalledWith("thread-1");
        expect(closeSideChats).toHaveBeenCalledWith("thread-2");
